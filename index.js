...
apifiles.forEach(file => {
  ...
  if (typeof apifile.load === 'function') {
    ...
    const apifiles1 = fs.readdirSync('./api').filter(file1 => file.endsWith('.js'));
    console.error(`'load' function not found in ${file}`);
  }
});
console.log(chalk.gray("+ ") + chalk.cyan("[") + chalk.white("Hclient") + chalk.cyan("]") + chalk.white(" APIs has loaded ✔️ "));
console.log(" ");
